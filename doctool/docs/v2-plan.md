...
- [ ] `LoadCategoriesFromSettings` 関数追加（Module1.bas）: 設定シートからカテゴリリスト取得
- [ ] `SortDictionaryKeys` 関数追加（Module1.bas）: Dictionary のキーソート
- [ ] Sheet1.cls: カテゴリ列定数を削除（定数定義部）
- [ ] Sheet1.cls: レビュー結果シート生成時にカテゴリ列ヘッダー行（9行目 B列〜）へエイリアスを書き込む処理追加
- [ ] Sheet1.cls: レビュー結果シート生成時のカテゴリリストチェック処理追加（不一致時 E13 エラー）
- [ ] Sheet1.cls: レビュー記録一覧初回作成時にカテゴリ列ヘッダー行（1行目 AC列〜）へエイリアスを書き込む処理追加
- [ ] Sheet1.cls: レビュー記録一覧転記時のカテゴリリストチェック処理追加（不一致時 E13 エラー）
- [ ] テスト: v2.0 テストケース シナリオ1〜5 の実行・確認

#### Phase 4: 設定シート拡張

- [ ] 指摘分類マッピング設定シートに 30 種類のサンプルカテゴリを追加（a〜z, aa〜ad）
- [ ] C 列（表示順）を設定して正しい順序でソートされることを確認
- [ ] テスト: 30 種類設定状態での抽出処理の正常動作確認
