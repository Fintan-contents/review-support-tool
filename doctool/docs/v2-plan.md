# 動的カテゴリ対応 設計書（2026-03-31）

> このドキュメントは **2026-03-31 動的カテゴリ対応の設計書**です。Phase 3 は実装済みです。
...
Phase 2: 可変長対応（1-2文字）（1.7日） ✅ 実装済み
  ↓ （カテゴリ抽出の拡張性を確保）
Phase 3: 動的列生成（1.3日）  ✅ 実装済み
  ↓ （カテゴリ数を可変にする）
Phase 4: 設定シート拡張（0.3日）  対象外（デフォルトは9カテゴリのまま）
  ↓ （30種類のサンプルカテゴリを設定）
```
...
sequenceDiagram
    ...
    RI->>T: 3. 指摘事項抽出実行（2回目）
    Note over T: ⚠️ カテゴリ構成チェック: 不一致<br/>ERR-004 エラー終了
    T-->>RI: ERR-004「カテゴリ設定が変更されました」
...
1. **エラー終了**:
   ...
   - ケース1（カテゴリ数変更）と同じ ERR-004 メッセージを表示
...
|--------|-----------|-------------|------------|
| ERR-002 | 指摘分類マッピング設定シートにカテゴリが登録されていません。 | 設定シートにカテゴリが 1 件もない | 処理中断 |
| ERR-004 | カテゴリ設定が変更されました。（詳細メッセージは ERR-004 参照） | 既存レビュー結果シートまたはレビュー記録一覧とカテゴリ構成が不一致 | 処理中断 |
...
| C | カテゴリ変更（追加または削除）でERR-004が発火し処理中断する | 2 |
| D | カテゴリ順序変更（数は同じ）でもERR-004が発火する | 3 |
| E | ERR-004後、結果シート削除→再実行で正常復帰する | 2 |
| F | やり直し時、レビュー記録一覧は削除不要（同一行が上書きされる） | 2 |
| G | 同回数再実行時、確認ダイアログなしで結果シート・記録票行・記録一覧行が自動上書きされる | 4 |
| H | 未登録カテゴリ使用時はエラーシートに記録して処理続行する | 5 |
| I | 設定シートが空の場合はERR-002で処理中断する | 5 |
...
| 4 | レビュー回数=2で実行 | ERR-004 表示・処理中断（観点C） |
| 5 | 「レビュー結果1回目」シートを削除 | - |
...
| 4 | レビュー回数=2で実行 | ERR-004 表示・処理中断（観点D）<br>カテゴリリスト "a,b,c," と "c,a,b," の不一致で検出 |
...
| 2 | 設定シートのカテゴリをすべて削除して実行 | ERR-002 表示・処理中断（観点I） |
