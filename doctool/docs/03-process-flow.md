...
|-------------|---------------------|--------------|-------|
| 1回目の抽出 | A1 のみ | 2026/03/01 | レビュー結果1回目シート |
| 2回目の抽出 | A1 と A2 | A2 = 2026/03/10（最後） | レビュー結果2回目シート |
...
- レビュー回数、日時情報を固定位置に書き込み

**サマリ転記**:

- レビュー回数列が空、または現在のレビュー回数と同じ行を探す
- 行が見つからない場合はテンプレート行をコピーして挿入
