...
集計のたびに以下の 2 ステップで「レビュー結果」列の値を決定する。

##### ステップ 1: 条件付合格判定（`DetermineReviewResult` 関数）

`CbConditional` チェックボックスが ON の場合にのみ実行する。

レビュー結果シート（`[処理対象設計書] レビュー結果N回目`）の **未済件数行**（`ROW_CATEGORY = 10` 行目）を走査し、以下の **全条件** を満たす場合に `"条件付合格"` を返す。

| 条件 | 詳細 |
|------|------|
| ① 指摘件数のある分類が1種類だけ | 未済件数 > 0 のカテゴリが、設定した「条件付合格の指摘分類」（名前付き範囲 `CONDITIONAL_CATEGORY`）のみ |
| ② 件数が上限以下 | そのカテゴリの未済件数 ≤ 名前付き範囲 `CONDITIONAL_COUNT` で設定した上限値 |
| ③ 未登録カテゴリエラーなし | 抽出中に発生したエラー件数（`errcount`）= 0 |

上記を1つでも満たさない場合は空文字列 `""` を返す。

##### ステップ 2: 合格判定（`WriteToReviewList` 内）

レビュー記録一覧シートに各カテゴリの未済件数を書き込んだ後、以下を評価する。

| 条件 | 書き込む値 |
|------|-----------|
| 未済件数の合計（`DETAIL_COL_LIST_CATEGORY_TOTAL` の SUM 数式）= 0 かつ errcount = 0 | `"合格"` |
| 上記以外 | ステップ 1 の戻り値（`"条件付合格"` または `""`）をそのまま書き込む |

**判定結果のまとめ**:

```
未済合計 = 0 かつ errcount = 0
    → 合格

CbConditional = TRUE
    かつ 未済件数が CONDITIONAL_CATEGORY のカテゴリのみ (他カテゴリの未済 = 0)
    かつ そのカテゴリの未済件数 ≤ CONDITIONAL_COUNT
    かつ errcount = 0
    → 条件付合格
...
カテゴリ増減時の動作は以下の原則に基づいて設計されている。

##### 原則1: カテゴリ数の固定（設計書Excel全体）

- 設計書 Excel の初回レビュー実行時にカテゴリ数を固定する
- `[処理対象設計書] レビュー結果N回目` と `[レビュー記録サマリ] レビュー記録一覧` の両方で同じカテゴリ数を使用する
- カテゴリが変更された場合はエラー終了し、やり直しを促すことでデータ整合性を保証する

##### 原則2: シンプルな実装

- 動的拡張・列シフトなどの複雑な処理を避ける
- カテゴリリスト（順序付き）の文字列比較のみで判定し、数・順序・内容の変更を一括検出する

##### 原則3: ユーザーへの明確な通知

- カテゴリ変更が発生した場合はエラーメッセージを表示する
- やり直し方法（設計書 Excel 全体のやり直し）を明示する

##### 原則4: 品質評価の観点を重視

- 複数のレビュー結果シートを横並びで比較・集計できるようにする
- カテゴリ数が異なるレビュー結果シートが混在すると集計不可になるため、設計書 Excel 全体で一貫したカテゴリ構造を維持する
