...
レビュー結果列・再レビュー方法列は定数を持たず、`WriteToReviewList` 内で以下の動的計算式を使用する:

- **REVIEW_RESULT 列**: `DETAIL_COL_LIST_CATEGORY_A + categoryMappings.Count`
- **RE_REVIEW_WAY 列**: `DETAIL_COL_LIST_CATEGORY_A + categoryMappings.Count + 1`
...
> `DETAIL_COL_LIST_REVIEW_RESULT = 38` / `DETAIL_COL_LIST_RE_REVIEW_WAY = 39` の固定定数はカテゴリ10以上で競合するバグがあったため削除し、動的計算式に変更した（S37/S40/S41 で検証済み）。
...
| 6 | 2026-03-16 | 1.5 | Claude Code | バグ修正完了を反映：DETAIL_COL_LIST_REVIEW_RESULT / RE_REVIEW_WAY 固定定数を削除し、動的計算式に変更（S37/S40/S41 で PASS 確認済み） |
