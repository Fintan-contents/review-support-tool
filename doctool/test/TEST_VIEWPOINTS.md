...
| ID | 観点 | 対応シナリオ |
|----|------|------------|
| 82 | レビュー記録一覧が未保存で開かれている場合に C01 ダイアログが表示され、「はい」で処理続行・「いいえ」で処理中断する | S44 |
...
| ID | 観点 | 対応シナリオ |
|----|------|------------|
| 83 | エラー（E01〜E09 等）発生後、`ScreenUpdating=True`・`Cursor=xlDefault`・`StatusBar=False` に復旧される | S45 |
...
| ID | 観点 | 対応シナリオ |
|----|------|------------|
| 84 | 件数が閾値を超過（件数 > `CONDITIONAL_COUNT`）する場合、`DetermineReviewResult` は `""` を返し REVIEW_RESULT 列が空欄になる | S46 |
...
| ID | 観点 | 対応シナリオ |
|----|------|------------|
| 85 | 複数の設計書を同時に開いて抽出した場合、レビュー記録一覧に各設計書の行が正しく追加され、カテゴリ別件数が正確に集計される | S47 |
...
- **OpenReviewListFile の既存ファイルクローズ確認（C01）**: → **VP82 / S44（手動）で対応済み**
- **CleanupOnError の画面復旧**: → **VP83 / S45（手動）で対応済み**
...
- **条件付合格の閾値境界値**: → **VP84 / S46 で対応済み**（閾値超過ケースを追加）
- **複数設計書同時処理の集計**: → **VP85 / S47 で対応済み**
