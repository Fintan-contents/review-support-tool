...
## OpenReviewListFile の動作検証

レビュー記録一覧ファイルのオープン処理（バックアップ作成・既存ブックのクローズ確認）を検証する。

### バックアップ機能

| ID | 観点 | 対応シナリオ |
|----|------|------------|
| 81 | `CbBkup=True` でレビュー記録一覧のバックアップファイル（`原ファイル名_YYYYMMDD_HHMMSS.xlsx`）が作成される | S43 |

### 既存ファイルクローズ確認（C01）

レビュー記録一覧がすでに Excel で開かれており、かつ未保存状態（`Saved = False`）の場合に C01 ダイアログが表示される。

| ID | 観点 | 対応シナリオ |
|----|------|------------|
| 82 | レビュー記録一覧が未保存で開かれている場合に C01 ダイアログが表示され、「はい」で処理続行・「いいえ」で処理中断する | S44（A: はい）, S45（B: いいえ） |

---

## CleanupOnError の画面復旧

エラー発生後に `Application.ScreenUpdating / Cursor / StatusBar` が正常にリセットされることを検証する。

| ID | 観点 | 対応シナリオ |
|----|------|------------|
| 83 | エラー（E01〜E09 等）発生後、`ScreenUpdating=True`・`Cursor=xlDefault`・`StatusBar=False` に復旧される | S46 |

---

## 条件付合格の境界値

`DetermineReviewResult` における条件付合格判定の境界値（件数=閾値・件数>閾値）を検証する。

S41 でカバーされる境界値（件数 ≤ 閾値 → 条件付合格）に加え、閾値超過ケースを検証する。

| ID | 観点 | 対応シナリオ |
|----|------|------------|
| 84 | 件数が閾値を超過（件数 > `CONDITIONAL_COUNT`）する場合、`DetermineReviewResult` は `""` を返し REVIEW_RESULT 列が空欄になる | S47 |

---

## 複数設計書同時処理の集計

複数の設計書を同時に開いて抽出した場合に、レビュー記録一覧への複数行追加・集計が正確であることを検証する。

| ID | 観点 | 対応シナリオ |
|----|------|------------|
| 85 | 複数の設計書を同時に開いて抽出した場合、レビュー記録一覧に各設計書の行が正しく追加され、カテゴリ別件数が正確に集計される | S48 |
...
| VBA機能領域 | コメント解析 | 結果シート生成 | 記録票転記 | ファイルフィルタ | ダイアログ | 連続/重複実行 | 削除 | エラー処理 | バリデーション | 警告 | カテゴリ動的調整 | 大量カテゴリ | REVIEW_RESULT列 | バックアップ | CleanupOnError | 複数設計書集計 |
|------------|:-----------:|:------------:|:---------:|:--------------:|:---------:|:-----------:|:---:|:---------:|:------------:|:---:|:---------------:|:----------:|:---------------:|:-----------:|:--------------:|:------------:|
| LoadBasicSettings | - | - | - | - | - | - | - | - | - | - | - | - | - | - | - | - |
| LoadCategoryMappings | VP48 | - | - | - | - | - | - | - | VP67,72,78,79 | - | - | - | - | - | - | - |
| LoadItemMappings | - | - | - | - | - | - | - | - | - | - | - | - | - | - | - | - |
| ValidateReviewListInputs | - | - | - | - | - | - | - | - | VP37-41 | - | - | - | - | - | - | - |
| OpenReviewListFile | - | - | - | - | VP82 | - | - | - | VP40-41 | - | - | - | - | VP81 | - | - |
...
| ExtractReviewComments | VP01-08,48 | VP10,13-15 | VP16,17 | - | - | - | - | VP25 | - | VP45-47 | VP62-64,70,71 | VP73 | - | - | - | VP85 |
| DetermineReviewResult | - | - | - | - | - | - | - | - | - | - | - | - | VP77,80,84 | - | - | - |
| WriteToReviewList | - | - | VP29 | - | - | - | - | - | - | VP45-47 | VP65,66 | VP75 | VP76,77,80 | - | - | VP85 |
| WriteToReviewRecordHeader/Summary | - | - | VP16,29 | - | - | - | - | - | - | - | - | VP75 | - | - | - | VP85 |
| GetReviewRecordSheets | - | - | VP18 | - | - | - | - | - | VP42 | - | - | - | - | - | - | - |
| ShowMsg / ダイアログ | - | - | - | - | VP21,22,30-36,82 | - | VP28 | - | VP37-44,67,72,78,79 | VP45-47 | - | - | - | - | - | - |
...
- **LoadItemMappings の直接検証**: 項目マッピング設定の読込エラー（セル参照不正等）を検証する観点がない。現在は E06（転記先シートなし）で間接的にカバー。
- **CalcPageVolume**: ページ数算出機能を直接検証する観点がない。レビュー記録一覧への転記時に間接的に使用されるが、計算結果の正確性を単独で検証するシナリオは未定義。
- **OpenReviewListFile のバックアップ機能**: → **VP81 / S43 で対応済み**
- **OpenReviewListFile の既存ファイルクローズ確認（C01）**: → **VP82 / S44〜S45（手動）で対応済み**（A: はい → S44、B: いいえ → S45）
- **CleanupOnError の画面復旧**: → **VP83 / S46（手動）で対応済み**
...
- **条件付合格の閾値境界値**: → **VP84 / S47 で対応済み**（閾値超過ケースを追加）
- **複数設計書同時処理の集計**: → **VP85 / S48 で対応済み**
