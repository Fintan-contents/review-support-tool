...
| 操作 | ツール | 理由 |
|------|--------|------|
| ビルド（VBA抽出・xlsmへの組み込み） | Makefile | WSL2 側のみで完結するため |
| テスト（VBAマクロ実行・Gold Master比較） | batファイル | Windows + Excel + pywin32 が必須のため |

**`make test` にしない理由**: WSL2 の Makefile から bat ファイルを呼び出すと「ラッパーのラッパー」になり、不必要に複雑になる。実行環境に合ったツールを直接使う方針を採用している。

- WSL2 側のみで完結する操作 → **Makefile**
- Windows 側の Excel 操作が必要な操作 → **batファイル**
...
# ---------------------------------------------------------
# テンプレートシートのアサーション（2026-03-31 カテゴリ動的調整の検証）
# ---------------------------------------------------------
...
| 手動テスト（`manual/`） | 21〜22, 30〜45 | ダイアログ操作・カテゴリ列動的調整（2026-03-31） |

**注**: 観点 21, 22（抽出前確認ダイアログの「いいえ」「キャンセル」動作）は、ユーザー選択による分岐が必要なため手動テストで実施。
自動テストでは testMode=True でダイアログをスキップし、全ファイル「はい」として処理する。

観点 30〜45（2026-03-31 カテゴリ列動的調整）は、テスト実行間でカテゴリ設定の変更が必要なため手動テストで実施。
