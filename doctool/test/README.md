...
| テスト方法 | 対象観点 | 備考 |
|----------|---------|-----|
| 自動テスト（`auto/`） | 01〜20, 23〜45 | Gold Master 比較＋ダイアログログ検証 |
| 手動テスト（`manual/`） | 21〜22 | ダイアログ操作（ユーザーによる分岐が必要） |

**注**: 観点 21, 22（抽出前確認ダイアログの「いいえ」「キャンセル」動作）は、ユーザー選択による分岐が必要なため手動テストで実施。
自動テストでは testMode=True でダイアログをスキップし、全ファイル「はい」として処理する。

観点 30〜45（v2.0 カテゴリ列動的調整）は、`config.yaml` の `setup.categories` および各 extract ステップの `categories` キーによって自動テストで実施。
シナリオ S12（カテゴリ増 9→12）および S13（カテゴリ減→増 9→6→9）が対応する。
...
│   ├── scenarioXX/        # 各シナリオフォルダ（scenario01〜scenario13）
