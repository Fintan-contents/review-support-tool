"""
S36 テストフィクスチャ生成スクリプト（30カテゴリ通常回帰テスト版）

S29 のフィクスチャをベースに、30カテゴリ（a～z, aa, ab, ac, ad）のコメントを
複数シートにまたがって挿入した S36 フィクスチャを生成する。

S35（702カテゴリ）はheavyタグによりデフォルトスキップ対象。
S36 は通常回帰テストとして毎回実行される軽量シナリオ。

使用方法:
    cd /home/kuma/work/review-support-tool/doctool/test
    source .venv/bin/activate
    python3 auto/scenario36/create_fixture.py
"""
...
def gen_s36_categories() -> list:
    """30カテゴリ（a-z: 26件 + aa, ab, ac, ad: 4件）を生成する。"""
    cats = list(string.ascii_lowercase)  # a-z: 26件
    cats.extend(["aa", "ab", "ac", "ad"])  # 2文字4件
    assert len(cats) == 30, f"Expected 30 categories, got {len(cats)}"
    return cats
...
def main():
    all_cats = gen_s36_categories()
    assert len(all_cats) == 30, f"Expected 30 categories, got {len(all_cats)}"

    # ---- 1. システム機能設計書 フィクスチャ作成 ----
    src_design = S29_DIR / "システム機能設計書_サンプル_S29.xlsx"
    dst_design = S36_DIR / "システム機能設計書_サンプル_S36.xlsx"
    ...
    if missing:
        print(f"WARNING: Missing categories ({len(missing)}): {sorted(missing)}")
    else:
        print("OK: All 30 categories are present.")

    # ---- 3. レビュー記録サマリ テンプレートコピー ----
    src_summary = S29_DIR / "レビュー記録サマリ_S29.xlsx"
    dst_summary = S36_DIR / "レビュー記録サマリ_S36.xlsx"
