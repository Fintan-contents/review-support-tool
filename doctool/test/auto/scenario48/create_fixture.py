"""
S48 テストフィクスチャ生成スクリプト（複数設計書同時処理の集計検証）

2つの設計書（A・B）を同時に開いて抽出し、
レビュー記録一覧に2行が正しく追加されることを検証する。

【フィクスチャの設計】
  設計書A（システム機能設計書_サンプルA_S48.xlsx）:
    - カテゴリ a: 1件（未済）
    - カテゴリ c: 1件（未済）
    - → レビュー記録一覧 行1: a未=1, b未=0, c未=1

  設計書B（システム機能設計書_サンプルB_S48.xlsx）:
    - カテゴリ b: 1件（未済）
...
Gold Master は以下の手順で作成:
  ...
  2. Windows 環境で S48 の全ファイルを開いて抽出を実行
...
def main():
    ...
    dst_design_a = S48_DIR / "システム機能設計書_サンプルA_S48.xlsx"
    create_design_doc_a(src_design, dst_design_a)

    # ---- 2. 設計書B: b(未) 1件 ----
    dst_design_b = S48_DIR / "システム機能設計書_サンプルB_S48.xlsx"
    ...
    dst_record_a = S48_DIR / "システム機能設計書_サンプルA_S48_レビュー記録票.xlsx"
    shutil.copy2(src_record, dst_record_a)
    print(f"Copied: {dst_record_a.name}")

    # ---- 4. レビュー記録票B テンプレートコピー ----
    dst_record_b = S48_DIR / "システム機能設計書_サンプルB_S48_レビュー記録票.xlsx"
