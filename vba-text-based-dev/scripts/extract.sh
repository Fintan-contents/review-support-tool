...
# 例: extract.sh ../doctool/config.mk
...
if [ -z "$1" ]; then
    ...
    echo ""
    echo "例:"
    echo "  extract.sh ../doctool/config.mk"
    echo "  extract.sh ../prtool/config.mk"
    echo ""
    exit 1
fi
...
# scriptsディレクトリから vba-text-based-dev ディレクトリに移動
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
VBA_DEV_DIR="$(dirname "$SCRIPT_DIR")"
