...
flowchart TB
    subgraph tool["ツール（doctool / prtool 等）"]
        xlsm["📊 *.xlsm<br/>VBA ツール本体"]
        subgraph tool_test["tool/test/"]
            tool_config["tool_config.yaml<br/>ツール固有設定"]
            subgraph scenarios["auto/scenarioXX/"]
                config_yaml["config.yaml<br/>テスト設定・観点"]
                fixtures["入力 .xlsx<br/>フィクスチャ"]
                gold_master["_expected.xlsx<br/>Gold Master<br/>（期待出力）"]
            end
        end
    end

    subgraph framework["test-framework（汎用・複数ツールで共用）"]
        auto_runner["auto_test_runner.py<br/>pytest アダプタ"]
        scenario_runner["scenario_runner.py<br/>VBA 実行・結果評価"]
        xlsx_diff["xlsx_diff.py<br/>差分比較エンジン"]
    end
...
flowchart TD
    bat(["run_auto_tests.bat"])
    --> pytest["pytest<br/>auto_test_runner.py<br/>シナリオを自動収集"]
    --> runner["scenario_runner.py<br/>シナリオごとに実行"]

    runner --> copy["フィクスチャを<br/>temp_dir/ にコピー"]
    copy --> excel["Excel を起動<br/>xlsm・入力 xlsx を開く"]
    excel --> setup["config.yaml の setup を適用<br/>（カテゴリ設定・名前付き範囲・<br/>ActiveX コントロール 等）"]
    setup --> vba["VBA マクロを実行<br/>（COM 経由・自動モード）"]
    vba --> save["出力ファイルを保存<br/>Excel を終了"]
    save --> eval["結果を評価"]

    eval --> gm["📊 Gold Master 比較<br/>_expected.xlsx との全シート差分"]
    eval --> msg["💬 ダイアログログ検証<br/>expected_messages 照合"]
    eval --> file["📋 ファイルアサーション<br/>assert_no_sheets 等"]
...
```mermaid
flowchart LR
    s1["① シナリオ作成<br/>config.yaml +<br/>入力フィクスチャ"]
    --> s2["② Gold Master 作成<br/>VBA 修正後の期待出力を<br/>_expected.xlsx として保存"]
    --> s3["③ テスト実行<br/>run_auto_tests.bat<br/>→ FAIL を確認（レッド）"]
    --> s4["④ VBA 実装<br/>vba_modules/ を編集<br/>build_vba.py でビルド"]
    --> s5["⑤ テスト実行<br/>run_auto_tests.bat<br/>→ PASS を確認（グリーン）"]
    --> s6["⑥ コミット<br/>vba_modules/ + xlsm<br/>+ Gold Master"]
```
